...
async function run() {
  ...
  if (Configuration.config.remove) {
    message = `Removing CDK Booster${Configuration.config.remove === 'all' ? ' including layer' : ''}...`;
  }
  ...
  if (Configuration.config.remove) {
    ...
    // delete folder .cdkbooster
    const folder = path.join(getProjectDirname(), '.cdkbooster');
    ...
    Logger.log('CDK Booster removed!');
        ...
        const confirn = await inquirer.prompt([
          {
            ...
            message: `\nThe following changes will be applied to your AWS account:${
              (changes.deployLayer
                ? `\n - Deploy CDK Booster layer version ${version}`
                : '') +
              (changes.lambdasToUpdate.length
