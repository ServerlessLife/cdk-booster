...
async function discoverLambdas() {
  ...
  if (!lambdasListNew) {
    if (noFramework) {
      throw new Error(
        `No framework was found, or Lambdas provided via ${configFileDefaultName} config file. If you are using monorepo, you should configure a subfolder via config/cli parameter or run the CDK Booster from a subfolder that contains the framework project.`,
      );
    } else {
      throw new Error(
