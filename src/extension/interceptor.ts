...
async function regularMode(context: any, event: any) {
  ...
  const timeout = setTimeout(() => {
    promiseResolve({
      statusCode: 500,
      body: "The function's in live debug mode but it hasn't heard back from your machine yet. If this is the first time you're trying out CDK Booster on this AWS account, AWS might need around 10 minutes to get everything ready. Hang tight and check back in a bit!",
    });
  }, 5 * 1000);
