...
async function addConfiguration() {
  try {
    ...
    if (!config) {
      Logger.error(
        'Failed to configure JetBrains IDE. Cannot find a locally installed CDK Booster. The JetBrains IDE debugger cannot use a globally installed version.',
      );
      return;
    }
