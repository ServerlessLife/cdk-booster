...
async function getLayerDescription() {
  if (!layerDescription) {
    layerDescription = `CDK Booster Layer version ${await getVersion()}`;
  }

  if ((await getVersion()) === '0.0.1') {
    // add a random string to the description to make it unique
    layerDescription = `CDK Booster Layer - development ${crypto.randomUUID()}`;
  }
    ...
    try {
      const updateFunctionConfigurationCommand =
        new UpdateFunctionConfigurationCommand({
          ...
          //Timeout: cdkbooster.argOptions.observable ? undefined : 300, // Increase the timeout to 5 minutes
...
/**
 * Get the policy document needed to attach to the Lambda role needed for the CDK Booster
 * @param roleName
 * @returns
 */
...
async function removeInfrastructure() {
  Logger.verbose('Removing CDK Booster infrastructure.');
  const promises: Promise<void>[] = [];
