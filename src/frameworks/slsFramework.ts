...
export class SlsFramework implements IFramework {
  ...
  public async getLambdas(config: LldConfigBase): Promise<LambdaResource[]> {
    ...
    try {
      ...
      Logger.log(
        'If you are running CDK Booster from a global installation, install Serverless Framework globally as well. If you are using monorepo, install Serverless Framework also in the project root folder. The fork of Serverless Framework https://github.com/oss-serverless/serverless is also supported.',
      );
