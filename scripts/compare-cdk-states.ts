 ...
 *
 * USAGE:
 *   npx tsx scripts/compare-cdk-states.ts <first-file.json> <second-file.json>
 *
 * PARAMETERS:
 ...
 *
 * EXAMPLES:
 *   npx tsx scripts/compare-cdk-states.ts cdk-original-function-files.json cdk-booster-function-files.json
 *   npx tsx scripts/compare-cdk-states.ts ./original.json ./booster.json
 *
 * EXPECTED INPUT FILES:
 * JSON files generated by list-lambda-files.ts with the structure:
 * {
...
/**
 * Interface representing the complete results output from list-lambda-files.ts
 */
 ...
 */
function compareCdkStates(firstFilePath: string, secondFilePath: string): ComparisonResults {
  ...
  for (const functionName of Array.from(allFunctionNames).sort()) {
    ...
    const { onlyInFirst, onlyInSecond, common } = compareArrays(firstFiles, secondFiles);
 ...
 */
function printResults(results: ComparisonResults, firstFile: string, secondFile: string): void {
  ...
  for (const func of results.functions) {
    ...
    if (!func.matches) {
      if (func.onlyInOriginal.length > 0) {
        console.log(`  Only in first file (${func.onlyInOriginal.length} files):`);
        func.onlyInOriginal.forEach(file => console.log(`    - ${file}`));
      }
      
      if (func.onlyInBooster.length > 0) {
        console.log(`  Only in second file (${func.onlyInBooster.length} files):`);
        func.onlyInBooster.forEach(file => console.log(`    + ${file}`));
      }
    }
    
    console.log(`  Common files: ${func.commonFiles.length}`);
    console.log(`  First total: ${func.originalFiles.length}, Second total: ${func.boosterFiles.length}\n`);
  }
...
async function main() {
  ...
  if (args.length !== 2) {
    console.error('Usage: npx tsx compare-cdk-states.ts <first-file.json> <second-file.json>');
    console.error('');
    console.error('Examples:');
    console.error('  npx tsx compare-cdk-states.ts cdk-original-function-files.json cdk-booster-function-files.json');
    console.error('  npx tsx compare-cdk-states.ts ./original.json ./booster.json');
    process.exit(1);
  }
