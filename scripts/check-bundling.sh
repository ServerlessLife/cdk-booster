...
BUNDLING_FOUND=$(grep -q "$BUNDLING_TEXT" "$LOG_FILE" && echo "true" || echo "false")

if [ "$SHOULD_BUNDLE" = "true" ]; then
    if [ "$BUNDLING_FOUND" = "false" ]; then
        echo "ERROR: Expected '$BUNDLING_TEXT' not found in $LOG_FILE"
        exit 1
    else
    ...
    if [ "$BUNDLING_FOUND" = "true" ]; then
        echo "ERROR: Unexpected '$BUNDLING_TEXT' found in $LOG_FILE"
        exit 1
    else
